...
contract Airdrop is Ownable {
    ...
    /// @notice events
    event SignatureClaim(address indexed to, uint256 amount);
    event MerkleClaim(address indexed to, uint256 amount);
    ...
    modifier canClaim(address _to) {
        require(msg.sender == _to, "Invalid claimer");
        require(alreadyClaimed[_to] == false, "Already claimed");
        _;
    }
    ...
    function signatureClaim(
        ...
        address _to,
        uint256 amount
    ) external canClaim(_to) {
        require(!isECDSADisabled, "SIGS_DISABLED");

        require(
            amount <= IERC20(macroToken).balanceOf(address(this)),
            "Insufficient token balance"
        );

        bytes32 typedData = toTypedDataHash(_to, amount);
        require(
            ECDSA.recover(typedData, signature) == signer,
            "Invalid signature"
        );

        /// @dev if we make it here, we have a valid signature and we can transfer the tokens
        alreadyClaimed[_to] = true;
        bool success = IERC20(macroToken).transfer(_to, amount);
        require(success, "Failed to transfer tokens");
        emit SignatureClaim(_to, amount);
    }
    ...
    function merkleClaim(
        ...
        /// @dev check Airdrop has sufficient MACRO token balance
        require(
            _amount <= IERC20(macroToken).balanceOf(address(this)),
            "Insufficient token balance"
        );

        bytes32 currentHash = toLeafFormat(_to, _amount);
        ...
        for (uint256 i = 0; i < _proof.length; i++) {
            ...
            if (currentHash < proofElement) {
                ...
                currentHash = keccak256(
                    bytes(abi.encode(currentHash, proofElement))
                );
            } else {
                // Hash(current element of the proof + current computed hash)
                currentHash = keccak256(
                    bytes(abi.encode(proofElement, currentHash))
                );
            }
        }

        // Check if the computed hash (root) is equal to the provided root
        require(currentHash == merkleRoot, "Invalid merkle proof");

        /// @dev transfer MACRO token to the claimer after setting their claim status to true
        alreadyClaimed[_to] = true;
        bool success = IERC20(macroToken).transfer(_to, _amount);
        require(success, "Failed to transfer tokens");
